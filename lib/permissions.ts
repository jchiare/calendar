...
export async function requireRole(allowed: Role[]): Promise<Actor> {
  ...
  const [result] = await db
    .select({ role: member.role, clinicId: clinics.id })
    .from(member)
    .innerJoin(clinics, eq(clinics.organizationId, member.organizationId))
    .where(
      and(eq(member.userId, session.user.id), eq(member.organizationId, organizationId)),
    )
    .limit(1);
  if (!result || !allowed.includes(result.role as Role))
